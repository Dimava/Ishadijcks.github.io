...
var addQuests = function(type, description, quests) {
	for (var i = 0; i < quests.length; i++) {
		quests[i].unshift(type, description)
...
var completeQuest = function(){
	if(questCompleted()){
		...
		if(player.curQuest.onclear)
			player.curQuest.onclear();
		else{
			startRandomQuest();
		}
	}
